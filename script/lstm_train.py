...
def main():
    ...
    parser.add_argument('--spei_month_type', type=str, default='12', help='which type SPEI to predict')
...
def plot(args, loss_dict, rmse_dict, mae_dict, r2_dict, epoch):
    ...
    plt.savefig(os.path.join(args.output_path, 'result',  f'SPEI{args.spei_month_type}_loss.png'))
    ...
    plt.tick_params(axis='both', labelsize=14)
    plt.savefig(os.path.join(args.output_path, 'result', f'SPEI{args.spei_month_type}_RMSE.png'))
    ...
    plt.tick_params(axis='both', labelsize=14)
    plt.savefig(os.path.join(args.output_path, 'result', f'SPEI{args.spei_month_type}_MAE.png'))
    ...
    plt.tick_params(axis='both', labelsize=14)
    plt.savefig(os.path.join(args.output_path, 'result', f'SPEI{args.spei_month_type}_R2.png'))
...
def save_excel(args, data, type):
    data = data.numpy()
    save_path = os.path.join(args.output_path, 'result', f'SPEI{args.spei_month_type}_{type}.xlsx')
...
def train(args):
    ...
    # model
    model = LSTMModel(args, infer=False).to(args.gpu_id)
